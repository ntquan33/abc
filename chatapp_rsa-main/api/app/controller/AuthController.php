...
class AuthController{
    ...
    public function signUp(){
        ...
        $public_key = mysqli_real_escape_string($this->conn->connect(), $_POST['public_key']);
        ...
        if(isset($_FILES['image'])){
           ...
           if(move_uploaded_file($tmp_name,"images/".$new_img_name)){
               ...
               $insert_query = mysqli_query($this->conn->connect(),
                   "INSERT INTO users (unique_id, fname, lname, email, password, img, status, public_key)
                         VALUES ({$ran_id}, '${fname}', '${lname}', '${email}', '${encrypt_pass}', '${new_img_name}', '${status}', '${public_key}')");
               if(!$insert_query){
